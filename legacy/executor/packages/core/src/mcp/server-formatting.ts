...
export function buildRunCodeDescription(tools?: ToolDescriptor[]): string {
  ...
  const discoverNote = "\n\nTooling tip: avoid repeated tiny discovery calls. Start with a single broad inventory pass via `tools.catalog.namespaces({})` and `tools.catalog.tools({ namespace?, query?, limit: 20 })`, then do at most one focused `tools.discover({ query, limit: 12, compact: true })`. `discover` returns `{ bestPath, results, total }`; prefer `bestPath` when present, otherwise use `results[i].path`. Use compact hints by default, and request exact schemas with `includeSchemas: true` when shaping payloads. Do not assign to `const tools = ...`; use a different variable name (e.g. `const discovered = ...`).";
