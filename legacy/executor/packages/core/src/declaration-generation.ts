...
export function generateToolInventory(tools: ToolDescriptor[]): string {
  ...
  return [
    ...
    "Then use `tools.discover({ query, limit?, compact?, includeSchemas? })` when you need ranking. It returns `{ bestPath, results, total }` (not an array).",
    "By default results are compact (`inputHint` / `outputHint`). Set `includeSchemas: true` when you need exact JSON Schemas (`typing.inputSchemaJson` / `typing.outputSchemaJson`).",
