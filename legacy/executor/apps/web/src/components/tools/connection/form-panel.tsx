...
import {
  ...
  parseAdditionalHeaderEntries,
  parseAdditionalHeaderRows,
  serializeAdditionalHeaderRows,
  type AdditionalHeaderEntry,
...
export function ConnectionFormPanel({
  ...
  const [headerRows, setHeaderRows] = useState<AdditionalHeaderEntry[]>(() => parseAdditionalHeaderEntries(""));
  const {
    sourceKey,
    scopeType,
    scopePreset,
  ...
  const handleSave = async () => {
    ...
    const parsedHeaders = parseAdditionalHeaderRows(headerRows);
    if (!parsedHeaders.value) {
      toast.error(parsedHeaders.error ?? "Invalid additional headers");
      return;
    }

    const serializedHeaders = serializeAdditionalHeaderRows(headerRows);
    setAdditionalHeadersText(serializedHeaders);
    ...
    try {
      const credentialScopeType = scope === "account"
        ...
        : scopeType === "organization"
          ? "organization"
          : "workspace";
      const additionalHeaders = Object.entries(parsedHeaders.value).map(([name, value]) => ({
        name,
        value,
      }));
  ...
  const updateHeaderRow = (index: number, patch: Partial<AdditionalHeaderEntry>) => {
    setHeaderRows((current) =>
